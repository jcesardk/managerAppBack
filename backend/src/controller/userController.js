...
module.exports = {
  ...
  async updateUsers(req, res) {
    ...
    const usuarios = await User.findByIdAndUpdate({ _id }, dataCreate, {
      new: true,
    });
    res.json(usuarios);
  },
  ...
  async associarAppUsario(req, res) {
    ...
    const aplicativo = await Aplicativo.findById(
      mongoose.Types.ObjectId(_idApp)
    );
    try {
      if (!aplicativo)
    ...
    const usuarioEdited = await User.findByIdAndUpdate(
      { _id: mongoose.Types.ObjectId(_idUser) },
      user
    );
